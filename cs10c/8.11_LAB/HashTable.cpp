...
int HashTable::computeHash(const string &s) {
    int value = 0; 
    for (char c : s) { //converts string to ACSII value
        value += c;
    }
    return value%size;
}
...
void HashTable::put(const string &s, int score) {
    int location = computeHash(s); //index of string
    for (list <WordEntry>::iterator i = hashTable[location].begin(); i != hashTable[location].end(); i++) {
        if (i->getWord() == s) {
            i->addNewAppearance(score); //just update values
            return;
        }
    }
    WordEntry entry(s,score);
    hashTable[location].push_back(entry);
}
