...
Node* newNode(string payload) { //creates a new node 
    /** fill in this code **/
   Node *temp = new Node();
...
Node* loadGame(int n, vector<string> names) {
    ...
    if (prev != nullptr) {
        ...
        prev->next = head; //connects last node to the front node
    }
    return head;
}
...
Node* runGame(Node* start, int k) { // josephus w circular list, k = num skips
    Node* curr = start;
    Node* prev = curr;
    while (curr->next != curr) { // exit condition, last person standing (checks if the node is last one)
