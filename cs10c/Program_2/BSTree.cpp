...
Node* BSTree::remove(Node *n, const string &key){
    ...
    // if(n->getLeftNode() != nullptr && n->getRightNode() != nullptr){ //case 1: 2 children
    //     Node *successor = n->getLeftNode();
    //     Node *successorParent = n;
    //     while(successor != nullptr && successor->getRightNode() != nullptr){ //finds successor node & parent
    //         successorParent = successor;
    //         successor = successor->getRightNode();
    //     }
    //     n->setLabel(successor->getLabel()); //copies successor value to node
    //     n->setCount(successor->getCount());
    //     remove(successor, successorParent);
    // } else if(n->getLeftNode() && n->getRightNode() == nullptr){ //case 4: no children
    //     delete n;
    //     n = nullptr;
    // } else if(n->getRightNode() == nullptr){ //case 2: if only left child
    //     if(parent == nullptr){ //if root node
    //         root = n->getLeftNode();
    //     } else if(parent->getLeftNode() == n){
    //         parent->setLeftNode(n->getLeftNode());
    //     } else {
    //         parent->setRightNode(n->getLeftNode());
    //     }
    // } else if(n->getLeftNode() == nullptr){ //case 3: if only right child
    //     Node *minimum = findMin(n->getRightNode());
    //     Node *minimumParent = n;
    //     n->setLabel(minimum->getLabel()); //copies minimum node
    //     n->setCount(minimum->getCount());
    //     minimum->decreaseCount();
    //     remove(minimum, minimumParent);
    // }
}
